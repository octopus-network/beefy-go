...
const RELAY_ENDPPOIT = "ws://127.0.0.1:9944"

// const RELAY_ENDPPOIT = "wss://rococo-rpc.polkadot.io"

const PARACHAIN_ENDPOINT = "ws://127.0.0.1:9988"
const PARACHAIN_ID = 2087

func TestUpdateClient(t *testing.T) {

	relayApi, err := gsrpc.NewSubstrateAPI(RELAY_ENDPPOIT)
	require.NoError(t, err)

	t.Log("==== connected! ==== ")

	// _parachainApi, err := client.NewSubstrateAPI("")
	// if err != nil {
	// 	panic(err)
	// }
	...
	for count := 0; count < 100; count++ {
		...
		case msg, ok := <-ch:
			...
			t.Logf("Recieved Signed Commitment count: #%d", count)
			...
			paraIds, err := beefy.FetchParaIDs(relayApi, blockHash)
			require.NoError(t, err)
			t.Logf("paraIds: $%#v", paraIds)
			...
			changeSet, err := beefy.QueryParaChainStorage(relayApi, PARACHAIN_ID, clientState.LatestBeefyHeight+1, blockNumber)
			require.NoError(t, err)
			...
			for _, changes := range changeSet {
				...
				for _, paraId := range paraIds {
					header, err := beefy.FetchParachainHeader(relayApi, paraId, changes.Block)
					require.NoError(t, err)
					heads[paraId] = header
				}
			...
			mmrBatchProof, err := beefy.GenerateBatchProof(relayApi, &blockHash, leafIndices)
			require.NoError(t, err)
			t.Logf("mmrBatchProof: %#v\n", mmrBatchProof)

			var parachainHeaders []*beefy.ParachainHeader
			...
			for i := 0; i < len(mmrBatchProof.Leaves); i++ {
				...
				for _, paraId := range sortedParaIds {
					bytes, err := codec.Encode(beefy.ParaIdAndHeader{ParaId: paraId, Header: paraHeaders[paraId]})
					require.NoError(t, err)
					leafHash := crypto.Keccak256(bytes)
					paraHeadsLeaves = append(paraHeadsLeaves, leafHash)
					if paraId == PARACHAIN_ID {
				...
				authorityRoot := beefy.Bytes32(v.Leaf.BeefyNextAuthoritySet.Root[:])
				parentHash := beefy.Bytes32(v.Leaf.ParentNumberAndHash.Hash[:])

				header := grandpa.ParachainHeader{
					ParachainHeader: paraHeaders[PARACHAIN_ID],
			...
			var payloadId beefy.SizedByte2 = CommitmentPayload.ID
			ParachainHeads := beefy.Bytes32(latestLeaf.ParachainHeads[:])
